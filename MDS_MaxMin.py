...
OUTPUT_PDF      = os.path.join(BASE_DIR, "MDS_scatterplot_MaxMin.pdf")
...
# Picker Exclusion: List antagonists to exclude from MAXMIN PICKING ONLY (they stay on the MDS plot)
MAXMIN_EXCLUDE_COMPOUNDS = []

# --- MaxMin Multi-Seed Settings ---
...
# === Step 4.5. Multi-Seed MaxMin Pick Frequency Loop ===
print(f"Running MaxMin over {NUM_SEEDS} random seeds...")
...
def dist_callback(i, j):
    return float(dist_matrix[eligible_indices[i], eligible_indices[j]])

picker = MaxMinPicker()
pick_size = min(NUM_TO_PICK, len(eligible_indices))
pick_counts = Counter()

if pick_size > 0:
    for seed in range(NUM_SEEDS):
        local_picks = picker.LazyPick(dist_callback, len(eligible_indices), pick_size, seed=seed)
...
print("✅ MaxMin pick frequency tally complete.")
...
plt.title(f"Classical MDS (MaxMin Selection Frequency over {NUM_SEEDS} Seeds)", fontsize=13)
